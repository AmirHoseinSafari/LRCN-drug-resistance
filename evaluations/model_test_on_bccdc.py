...
def load_data():
    ...
    print(len(index))
    print(len(missed))
    for i in range(4177, 4189):
        missed.append(i)
    print(missed)
    for i in range(len(test.columns)-1, -1, -1):
        for j in range(0, len(missed)):
            if missed[j] == i:
...
#     # X = np.array(X_val2)
#     for i in range(1, 9):
#         model = keras.models.load_model('../saved_models/DPAMR/deepamr' + str(i) + '.h5',
#                                         custom_objects={'masked_loss_function': masked_loss_function,
#                                                         'masked_accuracy': masked_accuracy})
#         y_pred_prob_tmp = model.predict(X, batch_size=32)
#         y_pred_prob = model.predict(X, batch_size=32)
#         th = model.class_threshold
...
def gbt_test(X,y):
    ...
    for i in range(0, 5):
        ...
        for j in range(0, len(drugs)):
            ...
            model = pickle.load(open('../saved_models/GBT/gbt'+str(drugs[j])+'_' + str(i) + '.sav', 'rb'))
            score_test, score_sr, score_pr = ROC_PR.ROC_ML(model, np.array(X_val2), np.array(y_val2), "GBT", 0,
                                                           xgb=True)
...
def lr_test(X,y):
    ...
    for i in range(0, 5):
        ...
        for j in range(0, len(drugs)):
            ...
            model = pickle.load(open('../saved_models/LR/lr'+str(drugs[j])+'_' + str(i) + '.sav', 'rb'))
            score_test, score_sr, score_pr = ROC_PR.ROC_ML(model, np.array(X_val2), np.array(y_val2), "LR", 0)
...
def svm_test(X,y):
    ...
    for i in range(0, 5):
        ...
        for j in range(0, len(drugs)):
            ...
            model = pickle.load(open('../saved_models/SVM/svm'+str(drugs[j])+'_' + str(i) + '.sav', 'rb'))
            score_test, score_sr, score_pr = ROC_PR.ROC_ML(model, np.array(X_val2), np.array(y_val2), "SVM", 0)
...
if __name__ == '__main__':
    ...
    print(test.shape)
    X, y, FrameSize = prepare_data_ml(test, label)
    svm_test(X, y)
